...
async function getprofile(req,res,next){
    try{
      ...
      if (getProfile.error) {
          response.responseHandler(res, STATUS.BAD_REQUEST, [], [{"message":getProfile.error.message}]);
      } else {    
          let profile = await CustomerService.getCustomerProfile(body.userId);
          response.responseHandler(res, STATUS.SUCCESS, [{"myprofile":profile}], [], SUCCESS.CUSTOMER_PROFILE, true);
      }
